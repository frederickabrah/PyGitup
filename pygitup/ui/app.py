...
class PyGitUpTUI(App):
    ...
    BINDINGS = [
        ...
        Binding("enter", "select_feature", "Launch", show=True),
    ]
    ...
    def on_mount(self) -> None:
        ...
        # Force focus to the list view immediately
        self.query_one("#feature-list").focus()

    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        """Updates the main content area when a list item is highlighted."""
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        """Handles Mouse Click and Enter key selection."""
        if event.item and isinstance(event.item, FeatureItem):
            self.launch_feature(event.item.mode)

    def action_select_feature(self) -> None:
        """Manual binding for Enter key if list view doesn't catch it."""
        list_view = self.query_one("#feature-list", ListView)
        if list_view.highlighted_child and isinstance(list_view.highlighted_child, FeatureItem):
    ...
    def launch_feature(self, mode: str):
        ...
        with self.suspend():
            try:
                # Clear screen before launching feature
