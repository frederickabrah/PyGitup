...
class PyGitUpTUI(App):
        ...
        yield Horizontal(
            ...
            ContentSwitcher(
                ...
                Vertical(
                    ...
                    Horizontal(
                        Input(placeholder="AI Message... (Click Generate)", id="ai-commit-msg", classes="form-input"),
    ...
    async def update_status_bar(self):
        """Pings GitHub to verify internet access."""
        import requests
        try:
            requests.get("https://github.com", timeout=5)
            self.is_online = True
            self.query_one("#status-bar").update("[bold][@click=app.sync]📡 ONLINE[/@click][/bold] | System Operational")
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        if event.list_view.id == "feature-list":
            ...
            elif mode == "ai-lab": self.run_ai_lab_view()
            ...
            elif mode == "search": self.run_search_view()
            ...
            else: self.notify(msg, severity="error")

    def run_mentor_view(self): 
        self.query_one("#main-switcher").current = "mentor-view"; self.query_one("#chat-input").focus()
        if not self.chat_history: self.query_one("#chat-log").write(RichMarkdown("# AI Assistant Initialized"))
    ...
    def run_ssh_view(self):
        ...
        if os.path.exists(key_path): self.query_one("#ssh-status-view").update(f"✅ **Key Found:** `{key_path}`")
        else: self.query_one("#ssh-status-view").update("❌ **No Key Found**")
    ...
    def run_ai_lab_view(self):
        ...
        log = self.query_one("#ai-diff-log"); log.clear()
        try:
            diff = subprocess.run(["git", "diff", "--cached"], capture_output=True, text=True).stdout
            if not diff: log.write("[yellow]No staged changes. Use 'git add' first.[/yellow]")
            else: log.write(RichMarkdown(f"### Staged Diff\n```diff\n{diff}\n```"))
        except Exception as e: log.write(f"[red]Error: {e}[/red]")

    async def on_input_submitted(self, event: Input.Submitted) -> None:
        if event.input.id == "chat-input":
            query = event.value; event.input.value = ""
            if self.pending_tool:
                tc = self.pending_tool; from ..utils.agent_tools import execute_agent_tool
                if tc['name'] == "github_issue" and tc['args'].get('action') == "comment":
                    tc['args']['body'] = query
                    self.query_one("#chat-log").write(RichMarkdown(f"💬 **Posting Comment...**"))
                    execute_agent_tool(tc['name'], tc['args'])
                    self.notify("Comment Posted"); self.pending_tool = None
                ...
                if query.lower() in ['y', 'yes']:
                    ...
                    self.run_worker(self.resume_agent_task(self.pending_tool, "User denied action."))
        ...
        while True:
            ...
            for tc in resp['tool_calls']:
                if tc['name'] in ["write_file", "patch_file", "run_shell"]:
                    ...
                    if tc['name'] in ["write_file", "patch_file"]:
                        ...
                        try:
                            ...
                            if tc['name'] == "patch_file":
                                # For patch, we simulate the result for the diff
                            ...
                            if diff_text:
                                ...
                                self.query_one("#chat-log").write(RichMarkdown(f"ℹ️ **No changes detected** for `{path}` (Content matches)."))
                        except Exception as e:
                            self.query_one("#chat-log").write(RichMarkdown(f"⚠️ **Diff Failed:** {e}"))
                    ...
                    self.query_one("#chat-loader").remove_class("-loading"); return
                result = execute_agent_tool(tc['name'], tc['args'])
    ...
    async def resume_agent_task(self, tool_call, result):
        self.chat_history.append({"role": "user", "text": "", "tool_results": [{"name": tool_call['name'], "content": result}]})
        await self.mentor_task("Proceed based on results.")
    ...
    async def diagnostic_task(self):
        ...
        config = load_config(); from ..utils.ai import ai_diagnostics_workflow
        result = ai_diagnostics_workflow(config, cmd)
        if isinstance(result, str): self.query_one("#diag-log").update(f"[red]Healing Proposal:[/red]\n{result}")
        elif result is True: self.query_one("#diag-log").update("[green]Passed![/green]")
    ...
    async def upload_task(self):
        ...
        log_widget = self.query_one("#project-log"); messages = ["[bold cyan]Deployment Sequence...[/bold cyan]"]
        ...
        try:
            ...
            if get_repo_info(user, name, token).status_code != 200: create_repo(user, name, token, description=desc, private=private)
            subprocess.run(["git", "push", "-u", "--force", f"https://{token}@github.com/{user}/{name}.git", "main"], check=True, capture_output=True)
            add_log("[green]SUCCESS! 🚀[/green]"); self.notify("Project Uploaded")
        except Exception as e: add_log(f"[red]Error: {e}[/red]")

    async def docs_task(self):
        config = load_config(); user, token = get_github_username(config), get_github_token(config)
        owner, repo = get_current_repo_context(); from ..project.docs import core_generate_docs
        generated = core_generate_docs(config, repo, "docs", user, token)
        self.query_one("#docs-log").update(f"[green]Success! Generated: {len(generated)} files.[/green]"); self.notify("Docs Generated")

    async def marketplace_task(self, btn_id):
        log = self.query_one("#market-log"); tpl_map = {"tpl-fastapi": "fastapi-pro", "tpl-node": "express-node", "tpl-cli": "cli-python"}
        tpl_name = tpl_map.get(btn_id); if not tpl_name: return
        config = load_config(); user, token = get_github_username(config), get_github_token(config)
        ...
        if success: log.update(f"[green]SUCCESS! {msg}[/green]"); self.notify("Template Deployed")
        else: log.update(f"[red]Failed: {msg}[/red]")

    async def ai_gen_task(self):
        log = self.query_one("#ai-lab-log"); config = load_config(); ai_key = config["github"].get("ai_api_key")
        from ..utils.ai import get_git_diff, generate_ai_commit_message
        diff = get_git_diff()
        if diff:
            msg = generate_ai_commit_message(ai_key, diff)
            if msg: self.query_one("#ai-commit-msg").value = msg; log.update("[green]Generated![/green]")
        else: log.update("[yellow]Nothing staged.[/yellow]")

    async def ai_commit_task(self):
        msg = self.query_one("#ai-commit-msg").value
        if msg:
            try: subprocess.run(["git", "commit", "-m", msg], check=True, capture_output=True); self.notify("Committed"); self.run_ai_lab_view()
            except Exception as e: self.query_one("#ai-lab-log").update(f"[red]Error: {e}[/red]")
    ...
    async def release_task(self):
        ...
        config = load_config(); user, token = get_github_username(config), get_github_token(config)
        owner, repo = get_current_repo_context(); from ..github.api import create_release
        resp = create_release(owner, repo, token, tag, name, notes)
        if resp.status_code == 201: self.notify("Release Created")
    ...
    async def create_gist_task(self):
        ...
        from ..github.api import github_request; config = load_config(); token = get_github_token(config)
        data = {"description": desc, "public": self.query_one("#gist-public").value, "files": {fname: {"content": content}}}
        if github_request("POST", "https://api.github.com/gists", token, json=data).status_code == 201: self.notify("Gist Created")

    async def list_gists_task(self):
        table = self.query_one("#gist-table", DataTable); table.clear(); config = load_config(); token, user = get_github_token(config), get_github_username(config)
        from ..github.api import github_request; resp = github_request("GET", f"https://api.github.com/users/{user}/gists", token)
        if resp.status_code == 200:
            for g in resp.json(): table.add_row(list(g['files'].keys())[0], g['description'] or "", "Public" if g['public'] else "Secret", g['html_url'])

    async def ssh_task(self):
        from ..utils.security import generate_ssh_key; from ..github.api import upload_ssh_key; config = load_config(); token = get_github_token(config)
        pub_key, path = generate_ssh_key(config["github"].get("email", "pygitup@user"))
        if pub_key and upload_ssh_key(token, f"PyGitUp - {os.uname().nodename}", pub_key).status_code == 201: self.notify("SSH Key Uploaded")

    async def list_prs_task(self):
        table = self.query_one("#pr-table", DataTable); table.clear(); config = load_config(); token = get_github_token(config); owner, repo = get_current_repo_context()
        from ..github.api import get_pull_requests; resp = get_pull_requests(owner, repo, token)
        if resp.status_code == 200:
    ...
    async def create_pr_task(self):
        ...
        config = load_config(); token = get_github_token(config); owner, repo = get_current_repo_context(); from ..github.api import create_pull_request
        if create_pull_request(owner, repo, token, title, head, base, body).status_code == 201: self.notify("PR Created")

    async def manage_pr_task(self, btn_id):
        table = self.query_one("#pr-table", DataTable); try: pr_num = table.get_row_at(table.cursor_row)[0]; except: return
        config = load_config(); token = get_github_token(config); owner, repo = get_current_repo_context(); from ..github.api import github_request
        if "merge" in btn_id: github_request("PUT", f"https://api.github.com/repos/{owner}/{repo}/pulls/{pr_num}/merge", token)
            ...
            self.query_one("#chat-input").focus(); self.notify("Enter comment in chat"); return
        self.run_worker(self.list_prs_task())

    async def gather_context_async(self):
        cwd = os.getcwd(); context = f"PATH: {cwd}\n"
        for root, dirs, files in os.walk("."):
            dirs[:] = [d for d in dirs if d not in [".git", "node_modules", "venv", "__pycache__", "dist"]]
            for f in files: context += f"- {os.path.relpath(os.path.join(root, f), '.')}\n"
        return context[:4000]
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        elif event.button.id == "btn-pr-mode-list": self.query_one("#pr-switcher").current = "pr-list-view"; self.run_worker(self.list_prs_task())
        ...
        elif event.button.id == "btn-gist-mode-list": self.query_one("#gist-switcher").current = "gist-list-view"; self.run_worker(self.list_gists_task())
        ...
        elif event.button.id.startswith("tpl-"): self.run_worker(self.marketplace_task(event.button.id))
        elif event.button.id == "btn-ai-gen": self.run_worker(self.ai_gen_task())
        elif event.button.id == "btn-ai-commit": self.run_worker(self.ai_commit_task())
        elif event.button.id == "btn-search-start": self.run_worker(self.search_task())
        elif event.button.id == "btn-scan": self.run_sast_scan()

    def run_search_view(self): self.query_one("#main-switcher").current = "search-view"
    ...
    def launch_cli_fallback(self, mode):
        ...
        with self.suspend():
            ...
            try:
                ...
                elif "template" in mode: create_project_from_template(user, token, config)
                else: print(f"Feature '{mode}' migrated to TUI."); input("\nPress Enter...")
